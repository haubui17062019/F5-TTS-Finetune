...
class CustomDatasetVi(Dataset):
    def __init__(
        ...
        with open(path_file_meta, "r", encoding="utf-8") as f:
            self.data = [line.strip().split("|") for line in f]
        self.path_folder_wav = path_folder_wav
        self.target_sample_rate = target_sample_rate
        self.hop_length = hop_length
        self.preprocessed_mel = preprocessed_mel
    ...
    def get_frame_len(self, index):
        file_name_wav, text = self.data[index][0], self.data[index][1]
        audio_path = f"{self.path_folder_wav}/{file_name_wav}"
        audio, source_sample_rate = torchaudio.load(audio_path)
        duration = audio.shape[-1] / source_sample_rate
